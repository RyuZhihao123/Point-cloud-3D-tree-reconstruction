...
void TreeWidget::paintGL()
{
...

    {
        ...
        m_meshProgram->setUniformValue("isTextured",m_isBarkTextured);
        m_meshProgram->setUniformValue("isLighting",true);
        m_meshProgram->setUniformValue("isBark",false);
        m_meshProgram->setUniformValue("isShowSeg",m_isShowSeg);
        m_tree->DrawTreeMeshVBO(m_meshProgram,m_modelMat);

        // leafs
        if(m_isShowLeaves)
        {
            setupShaders(m_leafProgram);
            glEnable(GL_TEXTURE0);
            m_leafProgram->setUniformValue("texture",0);
            m_leafTexture->bind();
            m_leafProgram->setUniformValue("viewPos",m_eyePos);
            m_leafProgram->setUniformValue("isTextured",1);
            m_leafProgram->setUniformValue("isLighting",false);
            m_leafProgram->setUniformValue("isBark",true);
            m_leafProgram->setUniformValue("isShowSeg",m_isShowSeg);
            m_tree->DrawLeafMeshVBO(m_leafProgram,m_modelMat);
        }
    }
